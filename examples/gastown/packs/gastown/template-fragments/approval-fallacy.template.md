...
- Continue with the next task, or send handoff context and exit:
  `{{ cmd }} mail send -s "HANDOFF: <brief>" -m "<context>" && gc runtime drain-ack && exit`
...
AUTO_PUSH=$({{ cmd }} bd show <work-bead> --json | jq -r '.[0].metadata | if has("auto_push") then (.auto_push | tostring) else "" end')
...
this session. `{{ cmd }} runtime drain-ack` makes the shutdown immediate. Polecats
